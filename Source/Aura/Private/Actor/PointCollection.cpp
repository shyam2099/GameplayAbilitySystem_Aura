...
TArray<USceneComponent*> APointCollection::GetGroundPts(const FVector& GroundLocation, int32 NumPts, float YawOverride)
{
	checkf(ImmutablePts.Num() >= NumPts, TEXT("Attempted to access ImmutablePts out of bounds"));

	TArray<USceneComponent*> ArrayCopy;

	for (USceneComponent* Pt : ImmutablePts)
	{
		if (ArrayCopy.Num() >= NumPts)
		{
			return ArrayCopy;
		}
