...
void UExecCalc_Damage::DetermineDebuff(const FGameplayEffectCustomExecutionParameters& ExecutionParams, const FGameplayEffectSpec& Spec, FAggregatorEvaluateParameters EvaluateParameters, const TMap<FGameplayTag, FGameplayEffectAttributeCaptureDefinition>& InTagsToDefs) const
{
	...
	for (TTuple<FGameplayTag, FGameplayTag> Pair : GameplayTags.DamageTypesToDebuffs)
	{
		...
		if (TypeDamage > -1.f)
		{
			...
			TargetDebuffResistance = FMath::Abs(TargetDebuffResistance);
                                              ...
                                              FGameplayEffectCustomExecutionOutput& OutExecutionOutput) const
{
	...
	for (const TTuple<FGameplayTag, FGameplayTag>& Pair : FAuraGameplayTags::Get().DamageTypesToResistances)
	{
		...
		{
			/*
			 * some shenanigans are afoot since radial damage is an unreal default function, and this needs to get into GAS damage calculations
			 * RadialDamageWithFalloff will call TakeDamage which can be overridden
			 * and a delegate can be called in the overridden function that will be bound here
			 */
	...
	const float ArmorPenCoefficient = ArmorPenCurve->Eval(SourceAvatarLevel);
	...
	const float CritResistCoefficient = CritResistCurve->Eval(TargetAvatarLevel);
